...
{ // A match was made.
   ...
   $rest_count = 0;
		...
		{	
			$outputtext .= "<div class='dontsplit'>\n";
			...
			for($i = 0; $i< count($rest_rows); $i++)
			{
				...
				$outputtext .= " <a onclick='".'submitForm(this,"viewrestaurantdiv&amp;loc_id='.$loc_id.'&amp;rest_id='.$rest_id.'")'."'>$name</a><br/>\n";
	...
	while($cat_row = mysqli_fetch_array($result_get_category, MYSQLI_ASSOC))
	{
		...
		if (count($rest_rows) > 0)//if Query is successfull 
		{
			...
			for($i = 0; $i< count($rest_rows); $i++)
			{
				...
				$name = htmlentities($info_row['name']);

				$rest_link[$name] = " <a onclick='".'submitForm(this,"viewrestaurantdiv&amp;loc_id='.$loc_id.'&amp;rest_id='.$rest_id.'")'."'>$name</a><br/>\n";
