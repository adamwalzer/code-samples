...
if($loc_id && $rest_id && $item_id)
{
	...
	if($input=$_POST['input'])
	{
		...
		if($order_data=$_POST['postdata'])
		{
			$delivery = $order_data['newdelivery'];
			$email = $order_data['newemail'];
			$phone = $order_data['newphonenumber'];
			$first_name = $order_data['newfirstname'];
			$last_name = $order_data['newlastname'];
			$del_address = $order_data['newaddress'];
			$del_city = $order_data['newcity'];
			$del_state = $order_data['newstate'];
			$del_zip = $order_data['newzipcode'];
			if($order_data['on_beach'])
			{
			...
			//$outputtext .= $delivery.','.$bill_name.','.$start_date.','.$total;
			...
			$query_insert_order = "
				INSERT INTO Customer_Order 
						(loc_id, rest_id, user_id, subtotal, tax_percent, tax, tip, total, first_name, last_name, phone, email, delivery, delivery_fee, delivery_minimum, pickup_minimum, del_on_beach, del_address, del_city, del_state, del_zip, bill_name, bill_address, bill_city, bill_state, bill_zip, start_date, last_edit, status)
			...
			*/
			//$_SESSION = mysqli_fetch_array($result_get_customer, MYSQLI_ASSOC);
    		...
    		$_SESSION = mysqli_fetch_array($result_get_customer, MYSQLI_ASSOC);//Assign the result of this query to SESSION Global Variable
