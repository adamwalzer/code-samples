...
if($order_data=$_POST['postdata'])
{
	if($_SESSION['order_id']!=0 && $_SESSION['order_id'])
	{
		...
		$del_address = $order_data['deliveryaddress'];
		$del_city = $order_data['deliverycity'];
		$del_state = $order_data['deliverystate'];
		$del_zip = $order_data['deliveryzipcode'];
		...
		$query_update_customer = "
			...
			SET total='$total', first_name='$first_name', last_name='$last_name', phone='$phone', email='$email', delivery='$delivery', delivery_fee='$delivery_fee', delivery_minimum='$delivery_minimum', del_address='$del_address', del_city='$del_city', del_state='$del_state', del_zip='$del_zip', last_edit='$last_edit' 
			WHERE order_id='$order_id'
			";
