			...
			if (@mysqli_num_rows($result_get_subcat) > 0)
			{
				...
				$outputtext .= "
					...
					<form id='itemForm' onSubmit='updateItemInOrder(".'"'.$order_item_id.'"'.");return false;' method='post' class='item_order_form' autocomplete='off'>
						<table id='itemtable'>
							<tr>
								<td colspan='5'>".htmlspecialchars($menu_item_row['descript'], ENT_QUOTES).$allergen."</td>
							</tr>
							<tr>
				...
				$query_get_group = "SELECT * FROM Customer_Order_Item_Option_Group WHERE (order_id='$order_id' AND order_item_id='$order_item_id') ORDER BY group_order";
				...
				while($group_row = mysqli_fetch_array($result_get_group, MYSQLI_ASSOC))
				{
					...
					$query_get_option = "SELECT * FROM Customer_Order_Item_Option WHERE (order_id='$order_id' AND order_item_id='$order_item_id' AND group_id='$group_id') ORDER BY option_order";
